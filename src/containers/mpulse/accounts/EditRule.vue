...
export default {
  ...
  if (this.numberOfRules < 2) {
    this.numberOfRules++
    this.$refs.ruleList.setRule(rule);
    }
  },
  removeRule() {
    ...
    onSubmit() {
      if (this.validate()) {
        this.$nextTick(() => {
          ...
          Vue.axios
            .post("/accounts/editAccountRule", params)
            .then(response => {
            ...
            .catch(error => {
              console.log(error)
              this.showErrorMsg({message: error.response.data.error});
            });
        });
      }
    },
  ...
  created() {
    ...
    console.log(this.accountRuleId)
    if(typeof(this.accountRuleId ) !== 'undefined' && this.accountRuleId !== null )
   {
      this.$store.dispatch('accounts/getAccountRuleInstance', {ruleid: this.accountRuleId})
      .then(response => {
        ...
        if (response.data.success == false) {
          ...
          this.form.distributor = response.data.distributor;
          this.form.account = response.data.account;
          this.form.ignoreinvoice = response.data.ignoreinvoice == 1 ? true : false;
          Vue._.forEach(response.data.rules, rule => {
            this.setRule(rule);
          }, this);
        }
      })
      .catch(error => {
        console.log(error)
        this.showErrorMsg({message: error.response.data })
      });
      ...
      this.showErrorMsg({
        message: "Please provide valid account rule to edit."
      })
      this.$emit("cancel");
    }
