...
export default {
  ...
  computed: {
    ...
    AccountTypeList() {
      let list = []
      Vue._.forEach(this.$store.state.AccountTypes.all, (AccountType) => {
        list.push({
    ...
    onSubmit() {
      if (this.validate()) {
        this.$nextTick(() => {
          ...
          this.$store.dispatch('accounts/editAccount', params)
            .then(() => {
              this.$emit('success')
              this.showSuccessMsg()
            })
  ...
  beforeMount() {
    ...
    this.form.tax = {label: this.account.tax.taxname, value: this.account.tax.taxid}
    this.form.accountType = {label: this.account.account_type.account_type, value: this.account.account_type.account_typeid}
  }
}
</script>
