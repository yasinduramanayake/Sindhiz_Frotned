...
export default {
  ...
  computed: {
    ...
    storeList() {
      ...
      Vue._.forEach(this.$store.state.stores.all, (store) => {
        ...
        if(store.storeid==this.day.storeid){
            this.form.store = {label: store.storename, value: store.storeid}
          }
      })
      return list
    },
    ...
    onSubmit() {
      if (this.validate()) {
        this.$nextTick(() => {
          ...
          this.$store.dispatch('orderDeliveryDays/editDay', params)
            .then(() => {
              this.$emit('success')
              this.showSuccessMsg()
            })
  ...
  beforeMount() {
    var dayIndex = _.findIndex(this.dayList, {value: this.day.day});
    this.form.distributor = {label: this.day.distributorname, value: this.day.distributorid}
    this.form.day = this.dayList[dayIndex]
    this.form.classification = this.day.classification
  },
  created () {
