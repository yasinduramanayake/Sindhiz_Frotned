          ...
          <b-col lg="12">
            ...
            <b-form v-on:submit.prevent="onSubmit" v-on:reset.prevent="onReset" novalidate>
              ...
              <b-form-group label="Sub Category" label-for="subCategory" :state="chkState('subCategory')">
                <v-select :options="subCategoryList" v-model="$v.form.subCategory.$model" placeholder="Select Sub Category"></v-select>
              ...
              <b-btn variant="outline-info" class="mb-2" @click="showCollapse = !showCollapse" ><i :class="showCollapse ? 'fa fa-minus fa-lg' : 'fa fa-plus fa-lg'"></i></b-btn>
              <b-collapse id="collapseOwner" v-model="showCollapse" >
                <b-form-group label="Packet Amount (Owner)" label-for="packetAmountOwner" >
                  <b-form-input id="packetAmountOwner"
                                type="number"
                ...
                <b-form-group label="Ctn Amount (Owner)" label-for="ctnAmountOwner" >
                  <b-form-input id="ctnAmountOwner"
                                type="number"
...
export default {
  ...
  computed: {
    ...
    subCategoryList() {
      ...
      Vue._.forEach(this.$store.state.subCategories.all, (subCategory) => {
        list.push({label: subCategory.subcat_name, value: subCategory.subcat})
      })
      return list
    },
    ...mapState({
    ...
    onSubmit() {
      if (this.validate()) {
        this.$nextTick(() => {
          ...
          Vue.axios.post('/addPromotion', {
            subCat: this.$v.form.subCategory.$model.value,
            startDate: this.$v.form.startDate.$model,
            endDate: this.$v.form.endDate.$model,
  ...
  created () {
    if (this.$store.state.subCategories.all.length == 0) {
