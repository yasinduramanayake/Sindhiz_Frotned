        ...
        <b-col lg="12">
          ...
          <b-form v-on:submit.prevent="onSubmit" v-on:reset.prevent="onReset" novalidate>
            ...
            <b-form-group label="Supplier Name" label-for="supplierName">
              <b-form-input id="supplierName" type="text" aria-describedby="liveFeedbackName" v-model.lazy.trim="$v.form.supplierName.$model" placeholder="Supplier Name" autofocus />
...
export default {
      ...
      if (this.validate()) {
        this.$nextTick(() => {
          ...
          let params = {
            ...
            supplierName: this.$v.form.supplierName.$model,
            isActive: this.$v.form.isActive.$model
          }

          this.$store.dispatch('suppliers/editSupplier', params)
            .then(() => {
              this.$emit('success')
              this.showSuccessMsg()
            })
  ...
  beforeMount() {
    ...
    this.form.estProfit = this.supplier.estprofit
    this.form.isActive = this.supplier.isactive == 1
    this.form.isVisible = this.supplier.isvisible == 1
  }
}
</script>
