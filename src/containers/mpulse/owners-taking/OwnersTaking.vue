...
<div className="animated">
  <b-card>
    <b-card-body>
      <b-form-group label="Store" label-cols-sm="4" label-cols-lg="3">
        <store-select :change="changeStore"></store-select>
      </b-form-group>
    </b-card-body>
  </b-card>
  ...
  <b-card v-show="selectedStore">
    ...
    <div slot="footer">
      ...
      <b-button class="mb-1" :disabled="!(totalSaleCash > 0 || extraCash > 0) || comment == '' "  type="submit" variant="primary" @click="submitBanking"><i class="fa fa-dot-circle-o"></i> Submit</b-button>
    </div>
  </b-card>

  <b-card>
    <taking-summary ref="takingSummary"></taking-summary>
  </b-card>
...
import TakingSummary from './TakingSummary'
export default {
  name: 'OwnersTaking',
  components: {
    TakingSummary
    ...
    submitBanking() {
      ...
      Vue.axios
        ...
        .then(() => {
          ...
          this.showSuccessMsg()
          this.resetForm()
          this.$refs.takingSummary.refresh()
        })
        .catch((error) => {
          this.showErrorMsg()
        })
