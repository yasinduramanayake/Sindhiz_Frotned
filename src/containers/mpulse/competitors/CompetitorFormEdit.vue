...
export default {
  ...
  computed: {
    ...
    CompetitorTypeList() {
      let list = []
      Vue._.forEach(this.$store.state.CompetitorTypes.all, (CompetitorType) => {
        list.push({
          label: CompetitorType.Competitortypename,
          value: CompetitorType.Competitortypeid
        })
      })
      return list
    },
    ...
    onSubmit() {
      if (this.validate()) {
        this.$nextTick(() => {
          this.submitted = true
          let params = {
            competitorId: this.competitor.competitorid,
            competitorName: this.$v.form.competitorName.$model,
          }
          this.$store.dispatch('competitors/editCompetitor', params)
            .then(() => {
              this.$emit('success')
              this.showSuccessMsg()
            })
  ...
  beforeMount() {
    this.form.competitorName = this.competitor.competitorname
  }
}
</script>
