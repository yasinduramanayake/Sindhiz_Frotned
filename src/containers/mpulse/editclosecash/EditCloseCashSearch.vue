  ...
  <div class="animated fadeIn">
    <b-card>
      <b-form v-on:submit.prevent="onSubmit" v-on:reset.prevent="onReset" novalidate>
        <b-row>
          <b-col sm="4">
            <b-form-group label="Store Name" label-for="store" :state="chkState('closingStore')" >
              <v-select :options="storeList"  v-model="$v.form.closingStore.$model" placeholder="Select Store"></v-select>
            </b-form-group>
          </b-col>
          <b-col sm="4">
    ...
    methods: {
      ...
      onSubmit() {
        if (this.validate()) {
          this.$nextTick(() => {
            let params = {
              closingStore: this.$v.form.closingStore.$model.value,
