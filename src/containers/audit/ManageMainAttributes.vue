  ...
  <div>
    <b-card>
      ...
      <b-sidebar
        ...
        shadow
      >
        <CreateMainAttributesModal @sidebar="hideSideBar" />
      </b-sidebar>
      ...
      <b-row>
        ...
        <b-col cols="12">
          ...
          >
            <template #cell(audit_main_attribute)="data">
        ...
        <b-modal
          ref="modalEditMainAttributes"
          hide-footer
          size="lg"
          no-enforce-focus
        >
          <UpdateMainAttributesModal
...
<script>
import CreateMainAttributesModal from "@/containers/audit/Components/AddMainAttributes.vue";
import UpdateMainAttributesModal from "@/containers/audit/Components/UpdateMainAttributesModal.vue";
import {
  BTable,
  BModal,
  BRow,
...
export default {
  ...
  methods: {
    async index(reset = false) {
      ...
      const res = await MainAttributeApi.getAllMainAttributes(
        this.form,
        this.perPage,
        this.currentPage
      );
    ...
    async hideSideBar() {
      this.$refs.sidebarAddMainAttributes.hide();
      await this.index(true);
    },
    showModal(item) {
      this.$refs.modalEditMainAttributes.show();
      this.selectedAttribute = item;
    },

    async hideModal() {
      this.$refs.modalEditMainAttributes.hide();
      await this.index(true);
    },
